...
app.post('/api/ask-sai', async (req, res) => {
  try {
    ...
    res.status(500).json({
      content: "I am having trouble connecting right now. Please remember: Shraddha and Saburi.",
...
app.post('/api/mood-guidance', async (req, res) => {
  try {
    ...
    res.status(500).json({ content: "", teaching: "Why fear when I am here?" });
  }
});
...
app.post('/api/interpret-answer', async (req, res) => {
  try {
    const { answer, userContext } = req.body;
...
Maintain a humble, devotional tone. Do not change the original meaning of the guidance.
    ...
    res.json({ reflection: chat.choices[0]?.message?.content || "" });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).json({ reflection: "" });
  }
});
